        ...
        public static NewUpdateInfo CheckforUpdates(bool fire_events = true)
        {
            try
            {
                using (WebClient nc = new WebClient())
                {
                    ...
                    nc.Headers.Add(HttpRequestHeader.IfNoneMatch, Program.Settings.UpdateSourceEtag);

                    string data = nc.DownloadString(UpdateURL);

                    Program.Settings.UpdateSourceEtag = nc.ResponseHeaders[HttpResponseHeader.ETag];
