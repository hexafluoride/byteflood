...
namespace ByteFlood.Services
{
    ...
    public sealed class GeoIPCountry : IDisposable
    {
        ...
        public string GetCountryFullName(byte[] ip)
        {
            return CountryNames[this.FindIndex(ip)];
        }

        private int FindIndex(byte[] ip)
        {
            return Convert.ToInt32(FindCountryCode(0, AddressToLong(ip), 31));
        }

        // Converts an IPv4 address into a long, for reading from geo database
        private long AddressToLong(byte[] ip)
        {
            long num = 0;
            for (int i = 0; i < 4; ++i)
            {
